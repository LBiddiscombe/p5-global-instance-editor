<script lang="ts">
	...
	let project = {
		name: 'My Project',
		files: [
			{ name: 'sketch.js', content: 'console.log("Hello, world!")' },
			{ name: 'class.js', content: '' }
		],
		editorIndex: 0,
		persist: true
	};
	...
	function renameable(node: HTMLElement, idx: number) {
		function handleKeyDown(event: KeyboardEvent) {
			if (event.key === 'Enter') {
				...
				project.files[idx].name = node.textContent ?? '';
				menuElement?.removeAttribute('open');
			}
			if (event.key === 'Escape') {
				event.preventDefault();
				node.textContent = project.files[idx].name;
				menuElement?.removeAttribute('open');
			}
		}
...
<div class="flex h-80 flex-grow flex-col rounded-2xl border border-base-content bg-base-200 p-2">
		...
		{#each project.files as file, idx}
			<li class="group relative flex gap-1">
				<button
					...
					class:font-medium={idx == project.editorIndex}
					class:bg-white={idx == project.editorIndex}
	...
	<p class="w-64 rounded-lg bg-yellow-300 p-2 text-center text-sm">
		Under Construction - Not hooked up to editor, try examples for now
	</p>
</div>
